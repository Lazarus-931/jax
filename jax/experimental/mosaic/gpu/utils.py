...
def shfl_up(x: ir.Value, distance: int | ir.Value):
  i32 = ir.IntegerType.get_signless(32)
  if isinstance(distance, int):
    distance = c(distance, i32)
  if (result_type := x.type) != i32:
    if (x_bitwidth := bitwidth(x.type)) < 32:
      ...
      for i in range(num_words):
        ...
        y_elem = shfl_up(x_elem, distance)
        y = vector.insert(
            y_elem,
            y,
            dynamic_position=[],
...
def shfl_idx(x: ir.Value, src_lane: int | ir.Value):
  i32 = ir.IntegerType.get_signless(32)
  if isinstance(src_lane, int):
    src_lane = c(src_lane, i32)
  if (result_type := x.type) != i32:
    if (x_bitwidth := bitwidth(x.type)) < 32:
      assert 32 % x_bitwidth == 0
      x = bitcast(x, ir.IntegerType.get_signless(x_bitwidth))
      ...
      for i in range(num_words):
        ...
        y_elem = shfl_idx(x_elem, src_lane)
        y = vector.insert(
            y_elem,
            y,
            dynamic_position=[],
    ...
    x = bitcast(x, i32)
  y = nvvm.shfl_sync(
      c(0xFFFFFFFF, i32), x, src_lane, c(0x1F, i32), nvvm.ShflKind.idx
  )
  if (x_bitwidth := bitwidth(result_type)) < 32:
    bits_ty = ir.IntegerType.get_signless(x_bitwidth)
    y_vec = bitcast(y, ir.VectorType.get((32 // x_bitwidth,), bits_ty))
...
def shfl_down(x: ir.Value, distance: int | ir.Value):
  i32 = ir.IntegerType.get_signless(32)
  if isinstance(distance, int):
    distance = c(distance, i32)
  if (result_type := x.type) != i32:
    if (x_bitwidth := bitwidth(x.type)) < 32:
      ...
      for i in range(num_words):
        ...
        y_elem = shfl_down(x_elem, distance)
        y = vector.insert(
            y_elem,
            y,
            dynamic_position=[],
    ...
    x = bitcast(x, i32)
  y = nvvm.shfl_sync(
      c(0xFFFFFFFF, i32), x, distance, c(0x1F, i32), nvvm.ShflKind.down
  )
  if (x_bitwidth := bitwidth(result_type)) < 32:
    bits_ty = ir.IntegerType.get_signless(x_bitwidth)
    y_vec = bitcast(y, ir.VectorType.get((32 // x_bitwidth,), bits_ty))
